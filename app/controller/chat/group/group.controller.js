...
exports.joinStudentToGroup = async (req, res) => {
  ...
  try {
    groupModel.findOne({ Name: req.body.GroupName }).then(
      (group) => {
        if (group) {
          group.GroupMember.push(req.body.StudentId);
          group.save();
          res.send({
            message: "Student added to existing group: " + req.body.GroupName,
          });
        } else {
...
//Save Group Messages
exports.saveGroupMessages = async (req, res) => {
  try {
    groupModel.findOne({ _id: req.body._id }).then(
      (group) => {
        if (group) {
          const json = {SentBy : req.body.SentBy, Message : req.body.Message, SentOn : req.body.SentOn};
...
//Get Group Messages
exports.getGroupMessagesByGroup = async (req, res) => {
  try {
    const result = await groupModel.find({ _id: req.params.groupId }).populate({
      path: "Chats.SentBy",
      model: "user",
    });
    res.send(result);
    ...
    console.log("error:", error);
    res.send({
      message: "Error retrieving group msgs",
      error,
    });
  }
};
...
exports.deleteGroup = (req, res) => {
  groupModel
    .findByIdAndUpdate(
      req.params.groupId,
      { Is_deleted: true, Deleted_Date: new Date(), Is_active: false },
      { new: true }
    )
    .then((group) => {
      if (!group) {
